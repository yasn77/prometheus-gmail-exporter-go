...
func TestRecordMetrics(t *testing.T) {
	...
	recordMetrics(1, unreadGauge, totalGauge, []string{"INBOX"}, srv, stopCh)
...
func TestNewServer(t *testing.T) {
	registry := prometheus.NewRegistry()
	server := newServer(registry, ":2112")
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.tokenData != nil || tt.corruptJSON {
				tmpfile, err = ioutil.TempFile("", "token-*.json")
				if err != nil {
					t.Fatalf("Failed to create temp file: %v", err)
				}
				defer os.Remove(tmpfile.Name())
...
func TestCreateGmailService(t *testing.T) {
	...
	credFile, err := ioutil.TempFile("", "credentials-*.json")
	if err != nil {
...
func TestEmptyConfigFile(t *testing.T) {
	...
	if config.Interval != 0 {
		t.Errorf("Expected Interval to be 0, got %d", config.Interval)
	}
	if config.Labels != nil {
		t.Errorf("Expected Labels to be nil, got %v", config.Labels)
	}
}
...
func TestConfigValidation(t *testing.T) {
	tests := []struct {
		...
		{
			name: "positive interval with labels",
			config: Config{
				Interval: 300,
				Labels:   []string{"INBOX"},
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Config struct just holds data; validation logic will be added during security remediation
			// For now, just verify the struct was created correctly
...
func TestCreateGmailServiceWithTokenPathEnv(t *testing.T) {
	...
	tokenFile, err := ioutil.TempFile("", "custom-token-*.json")
	if err != nil {
		t.Fatalf("Failed to create temp token file: %v", err)
	}
	defer os.Remove(tokenFile.Name())
